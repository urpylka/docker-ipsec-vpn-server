...
# Wrote by Artem Smirnov <urpylka@gmail.com>
...
# TODO: can be used 0.0.0.0 or 255.255.255.255
check_ip() {
...
change_client(){
  ...
  if [[ -z ${SEARCH_RESP} ]];
  then echo "User was not found by the current request"
  else
    echo ${SEARCH_RESP}
    while true; do
...
used_route(){
  ...
  then echo "Subnetwork ${REMOTE_NETWORK_24} used"; return 0
  else echo "Subnetwork ${REMOTE_NETWORK_24} didn't use"; return 1
  fi
}

# ! used_route ${REMOTE_NETWORK_24
# echo "No IP from subnetwork ${REMOTE_NETWORK_24} in ${SECRETS_FILE}."

del_route(){
  REMOTE_NETWORK_24=$1
  if exists_route ${REMOTE_NETWORK_24};
  then
    echo "Found route to network ${REMOTE_NETWORK_24} в ${IPT_FILE}. Removing..."
    SEARCH_RESP="-A FORWARD -s ${REMOTE_NETWORK_24} -d ${REMOTE_NETWORK_24} -i ppp+ -o ppp+ -j ACCEPT"
...
# used_route ${REMOTE_NETWORK_24}
# echo "Found IP from subnetwork ${REMOTE_NETWORK_24} in ${SECRETS_FILE}."
...
add_route(){
  ...
  if ! exists_route ${REMOTE_NETWORK_24};
  then
    echo "Not found route in ${REMOTE_NETWORK_24} network in ${IPT_FILE}. Adding..."
